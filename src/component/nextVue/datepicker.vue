...
export default {
  ...
  watch: {
    ...
    now() {
      const { now } = this;
      this.current = _(now) ? _.timestamp(now) : now;
      this.init();
    },
    showTime(time) {
  ...
  computed: {
    getCurrent() {
      ...
      return _.dateFormat(format, current);
    },
  },
  methods: {
    init() {
    ...
    // 获得当前选择
    getNow(type) {
      return this[`now${_.firstUppercase(type)}`];
    },
    // 获得编辑状态
    getEditStatus(type) {
      return this[`hasEdit${_.firstUppercase(type)}`];
    },
    // 去编辑
    toEdit(type, has) {
    ...
    dayScope() {
      ...
      const firstWeek = _.week(`${currentYear}/${nowMonth}/1`);
      const min = 1;
