export default {
  ...
  mounted() {
    ...
    liliaState.setFlowAction('change', (status, no) => {
  ...
  methods: {
    selected(pageNo) {
      ...
      return currentNo === pageNo ? 'page-selected' : '';
    },
    getTotalPage() {
    ...
    inputPageNo(no) {
      this.liliaState.wheelFlowAction('change', Number(no));
    },
    // 触发选择
    tapPageNo(no) {
      this.liliaState.wheelFlowAction('change', no);
    },
    // 重置
    reset() {
      this.setPageNoScope();
    ...
    firstPageNo() {
      ...
      this.liliaState.wheelFlowAction('change', 1);
    },
    // 计算页面显示记录数量
    setRecordAmount() {
