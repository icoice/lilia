      ...
      <tr v-for="th in tbHead" class='tb-head'>
        <td class="tb-head-select" v-if='hasMulti'>
          <div v-if='tbSel.length === 0'>
            <btn @tap="allCheck">
              <span slot="btn" class="iconfont icon-blank"></span>
            </btn>
          </div>
          <div v-else>
            <btn @tap="backCheck">
              <span slot="btn" class="iconfont icon-check"></span>
            </btn>
          </div>
        </td>
        <td v-for="item in th"
      ...
      <tr v-for="(item, code) in tbData">
        <td class="tb-select" v-if='hasMulti'>
          <btn @tap="e => multiSelect(code)">
            <span slot="btn" class='iconfont icon-check' v-if='tbSel.indexOf(code) >= 0'></span>
            <span slot="btn" class='iconfont icon-blank' v-else></span>
          </btn>
        </td>
        ...
        <td class='tb-opt' v-if='tbOpts && tbOpts.length > 0' v-for="(opt, code) in tbOpts">
          <btn @tap='e => tap({ key: opt.key, name: opt.name, data: item, code })'>
            <div slot='btn'>
              <span class='iconfont icon-3dcube'></span>
